...
from rnn_common.np import *  # import numpy as np (or import cupy as np)
from rnn_common.layers import *
from rnn_common.functions import sigmoid
...
class TimeRNN:
    ...
    def forward(self, xs):
        Wx, Wh, b = self.params
        N, T, D = xs.shape #미니배치 크기 N, 데이터의 개수 T 입력벡터의 차원수 D
        D, H = Wx.shape
...
class LSTM:
    ...
    def forward(self, x, h_prev, c_prev):
        #x는 현 시간의 입력, h_prev는 이전 시각의 은닉상태, c_prev는 이전 시각의 기억셀이다.
        Wx, Wh, b = self.params
        N, H = h_prev.shape

        # 각 gate에 속하는 affine 변환을 행렬연산으로 한번에 표현
        ...
        f = sigmoid(f) #forget 게이트
        g = np.tanh(g) #실질적인 정보를 전달하는 게이트
        i = sigmoid(i) #input 게이트
        o = sigmoid(o) #output 게이트
    ...
    def backward(self, dh_next, dc_next):
        ...
        #주어진 게이트들의 가중치를 행렬로 묶어서, 다음 가중치 연산에 활용한다.
